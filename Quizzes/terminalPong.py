...
def main():
    ...
    ball_process = multiprocessing.Process(target=draw_screen,args=(height,width))#Multi process solution here
    ball_process.start()
    
    input_thread = threading.Thread(target=handle_input,daemon=True)#threaded solution here
    input_thread.start()
